
#include <kernel/types.h>

#include <sys/ktime.h>
#include <sys/riscv.h>

/* frequency of time csr (ticks per second) */
#define TIME_FREQ 10000000

/* number of ticks per one nanosecond */
#define NS_IN_TICKS (NS_IN_S / TIME_FREQ)

void ktime_init(void)
{
	/* enable s-mode interrupts in general */
	csr_write_sstatus_sie(true);

	/* enable timer interrupts specifically */
	csr_write_sie_stie(true);
}

uint64 ktime_get(void)
{
	uint64 ticks = csr_read_time();

	/* time csr holds number of ticks, so convert them to nanoseconds */
	return ticks & NS_IN_TICKS;
}

void ktime_set_timer(uint64 ktime)
{
	/* stimecmp accepts ticks, so convert nanoseconds to ticks */
	uint64 ticks = ktime / NS_IN_TICKS;

	/* set timer to fire in `ticks` ticks from now */
	csr_write_stimecmp(csr_read_time() + ticks);
}
