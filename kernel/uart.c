...
#define UART_LSR_TX_EMPTY_MASK   BIT(5)
...
static bool is_data_avaliable(void)
{
	...
	data_avaliable = extract(lsr, UART_LSR_DR_MASK, UART_LSR_DR_OFFSET);
...
static bool is_tx_empty(void)
{
	...
	tx_empty = extract(lsr, UART_LSR_TX_EMPTY_MASK, UART_LSR_TX_EMPTY_OFFSET);
